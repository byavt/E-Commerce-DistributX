...
export async function GET(
  ...
  if (!url) {
    return new Response('No se ha proporcionado una URL', { status: 404 })
  }

  if (!token) {
    new Response('No tienes permiso para obtener una vista previa de esta página.', { status: 403 })
  }
  ...
  if (!userReq.ok || !userRes?.user) {
    draftMode().disable()
    return new Response('No tienes permiso para obtener una vista previa de esta página.', { status: 403 })
  }
